...
import { checkTenantLimit } from '../middleware/limits.middleware';
...
router.post('/branches', requirePermission(PERMISSIONS.SETTINGS_UPDATE), checkTenantLimit('branches'), createBranch);
...
router.post('/users', requirePermission(PERMISSIONS.SETTINGS_USERS_CREATE), checkTenantLimit('users'), createUser);
